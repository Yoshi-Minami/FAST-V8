...
! =========  TwrPropsParms  =======
  TYPE, PUBLIC :: TwrPropsParms
    ...
    REAL(ReKi) , DIMENSION(:,:), ALLOCATABLE  :: TwrCL 
...
ENDIF
IF (ALLOCATED(SrcTwrPropsParmsData%TwrCL)) THEN
  i1_l = LBOUND(SrcTwrPropsParmsData%TwrCL,1)
  i1_u = UBOUND(SrcTwrPropsParmsData%TwrCL,1)
  i2_l = LBOUND(SrcTwrPropsParmsData%TwrCL,2)
  i2_u = UBOUND(SrcTwrPropsParmsData%TwrCL,2)
  IF (.NOT. ALLOCATED(DstTwrPropsParmsData%TwrCL)) THEN 
    ALLOCATE(DstTwrPropsParmsData%TwrCL(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstTwrPropsParmsData%TwrCL.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstTwrPropsParmsData%TwrCL = SrcTwrPropsParmsData%TwrCL
ENDIF
...
ENDIF
IF (ALLOCATED(TwrPropsParmsData%TwrCL)) THEN
  DEALLOCATE(TwrPropsParmsData%TwrCL)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  IF ( ALLOCATED(InData%TwrCL) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! TwrCL upper/lower bounds for each dimension
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TwrCL,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TwrCL,2)
    Int_Xferred = Int_Xferred + 2
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    ALLOCATE(OutData%TwrCL(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask2 = .TRUE. 
      IF (SIZE(OutData%TwrCL)>0) OutData%TwrCL = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%TwrCL))-1 ), mask2, 0.0_ReKi )
