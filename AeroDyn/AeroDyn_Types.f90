...
! =========  AD_InputFile  =======
  TYPE, PUBLIC :: AD_InputFile
    ...
    INTEGER(IntKi)  :: NumTwrAFfiles      !< Number of airfoil files used for tower sections [-]
    CHARACTER(1024) , DIMENSION(:), ALLOCATABLE  :: TwrAFNames      !< Tower Airfoil file names (NumAF lines) [quoted strings]
    ...
    INTEGER(IntKi)  :: NumTwrAlf      !< Length of the Alpha and Coefs arrays [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: TwrAoA      !< Attack of angle for tower aerofoil [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: TwrCl      !< Coefficient of lift at tower node [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: TwrCd      !< Coefficient of drag at tower node [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: TwrCm      !< Coefficient of pitching moment at tower node [-]
...
! =========  AD_MiscVarType  =======
  TYPE, PUBLIC :: AD_MiscVarType
    ...
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: X_TwrLift      !< local x-component of force per unit length of the jth node in the tower [m/s]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: Y_TwrLift      !< local y-component of force per unit length of the jth node in the tower [m/s]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: M_Twr      !< local pitching moment per unit length of the jth node in the tower [Nm/m]
...
! =========  AD_OutputType  =======
  TYPE, PUBLIC :: AD_OutputType
    ...
    TYPE(MeshType)  :: TowerLiftLoad      !< loads on the tower [-]
...
IF (ALLOCATED(SrcInputFileData%BladeProps)) THEN
  ...
  END IF
    ...
    ENDDO
ENDIF
    DstInputFileData%NumTwrAFfiles = SrcInputFileData%NumTwrAFfiles
IF (ALLOCATED(SrcInputFileData%TwrAFNames)) THEN
  i1_l = LBOUND(SrcInputFileData%TwrAFNames,1)
  i1_u = UBOUND(SrcInputFileData%TwrAFNames,1)
  IF (.NOT. ALLOCATED(DstInputFileData%TwrAFNames)) THEN 
    ALLOCATE(DstInputFileData%TwrAFNames(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInputFileData%TwrAFNames.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInputFileData%TwrAFNames = SrcInputFileData%TwrAFNames
ENDIF
...
ENDIF
    DstInputFileData%NumTwrAlf = SrcInputFileData%NumTwrAlf
IF (ALLOCATED(SrcInputFileData%TwrAoA)) THEN
  i1_l = LBOUND(SrcInputFileData%TwrAoA,1)
  i1_u = UBOUND(SrcInputFileData%TwrAoA,1)
  IF (.NOT. ALLOCATED(DstInputFileData%TwrAoA)) THEN 
    ALLOCATE(DstInputFileData%TwrAoA(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInputFileData%TwrAoA.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInputFileData%TwrAoA = SrcInputFileData%TwrAoA
ENDIF
IF (ALLOCATED(SrcInputFileData%TwrCl)) THEN
  i1_l = LBOUND(SrcInputFileData%TwrCl,1)
  i1_u = UBOUND(SrcInputFileData%TwrCl,1)
  IF (.NOT. ALLOCATED(DstInputFileData%TwrCl)) THEN 
    ALLOCATE(DstInputFileData%TwrCl(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInputFileData%TwrCl.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInputFileData%TwrCl = SrcInputFileData%TwrCl
ENDIF
...
ENDIF
IF (ALLOCATED(SrcInputFileData%TwrCm)) THEN
  i1_l = LBOUND(SrcInputFileData%TwrCm,1)
  i1_u = UBOUND(SrcInputFileData%TwrCm,1)
  IF (.NOT. ALLOCATED(DstInputFileData%TwrCm)) THEN 
    ALLOCATE(DstInputFileData%TwrCm(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInputFileData%TwrCm.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInputFileData%TwrCm = SrcInputFileData%TwrCm
ENDIF
...
ENDIF
IF (ALLOCATED(InputFileData%TwrAFNames)) THEN
  DEALLOCATE(InputFileData%TwrAFNames)
ENDIF
...
ENDIF
IF (ALLOCATED(InputFileData%TwrAoA)) THEN
  DEALLOCATE(InputFileData%TwrAoA)
ENDIF
IF (ALLOCATED(InputFileData%TwrCl)) THEN
  DEALLOCATE(InputFileData%TwrCl)
ENDIF
IF (ALLOCATED(InputFileData%TwrCd)) THEN
  DEALLOCATE(InputFileData%TwrCd)
ENDIF
IF (ALLOCATED(InputFileData%TwrCm)) THEN
  DEALLOCATE(InputFileData%TwrCm)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  IF ( ALLOCATED(InData%BladeProps) ) THEN
   ...
   ! Allocate buffers for subtypes, if any (we'll get sizes from these) 
    ...
    END DO
  END IF
      Int_BufSz  = Int_BufSz  + 1  ! NumTwrAFfiles
  Int_BufSz   = Int_BufSz   + 1     ! TwrAFNames allocated yes/no
  IF ( ALLOCATED(InData%TwrAFNames) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! TwrAFNames upper/lower bounds for each dimension
      Int_BufSz  = Int_BufSz  + SIZE(InData%TwrAFNames)*LEN(InData%TwrAFNames)  ! TwrAFNames
  END IF
  ...
  END IF
      Int_BufSz  = Int_BufSz  + 1  ! NumTwrAlf
  Int_BufSz   = Int_BufSz   + 1     ! TwrAoA allocated yes/no
  IF ( ALLOCATED(InData%TwrAoA) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! TwrAoA upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%TwrAoA)  ! TwrAoA
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! TwrCl allocated yes/no
  IF ( ALLOCATED(InData%TwrCl) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! TwrCl upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%TwrCl)  ! TwrCl
  END IF
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! TwrCm allocated yes/no
  IF ( ALLOCATED(InData%TwrCm) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! TwrCm upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%TwrCm)  ! TwrCm
  END IF
  ...
  ELSE
    ...
    END DO
  END IF
      IntKiBuf ( Int_Xferred:Int_Xferred+(1)-1 ) = InData%NumTwrAFfiles
      Int_Xferred   = Int_Xferred   + 1
  IF ( .NOT. ALLOCATED(InData%TwrAFNames) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TwrAFNames,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TwrAFNames,1)
    Int_Xferred = Int_Xferred + 2

    DO i1 = LBOUND(InData%TwrAFNames,1), UBOUND(InData%TwrAFNames,1)
        DO I = 1, LEN(InData%TwrAFNames)
          IntKiBuf(Int_Xferred) = ICHAR(InData%TwrAFNames(i1)(I:I), IntKi)
          Int_Xferred = Int_Xferred   + 1
        END DO ! I
    END DO !i1
  END IF
  ...
  END IF
      IntKiBuf ( Int_Xferred:Int_Xferred+(1)-1 ) = InData%NumTwrAlf
      Int_Xferred   = Int_Xferred   + 1
  IF ( .NOT. ALLOCATED(InData%TwrAoA) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TwrAoA,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TwrAoA,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%TwrAoA)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%TwrAoA))-1 ) = PACK(InData%TwrAoA,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%TwrAoA)
  END IF
  IF ( .NOT. ALLOCATED(InData%TwrCl) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TwrCl,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TwrCl,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%TwrCl)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%TwrCl))-1 ) = PACK(InData%TwrCl,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%TwrCl)
  END IF
  ...
  END IF
  IF ( .NOT. ALLOCATED(InData%TwrCm) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TwrCm,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TwrCm,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%TwrCm)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%TwrCm))-1 ) = PACK(InData%TwrCm,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%TwrCm)
  END IF
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    END DO
  END IF
      OutData%NumTwrAFfiles = IntKiBuf( Int_Xferred ) 
      Int_Xferred   = Int_Xferred + 1
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! TwrAFNames not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%TwrAFNames)) DEALLOCATE(OutData%TwrAFNames)
    ALLOCATE(OutData%TwrAFNames(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%TwrAFNames.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
    DO i1 = LBOUND(OutData%TwrAFNames,1), UBOUND(OutData%TwrAFNames,1)
        DO I = 1, LEN(OutData%TwrAFNames)
          OutData%TwrAFNames(i1)(I:I) = CHAR(IntKiBuf(Int_Xferred))
          Int_Xferred = Int_Xferred   + 1
        END DO ! I
    END DO !i1
    DEALLOCATE(mask1)
  ...
  ELSE
    ...
    DEALLOCATE(mask1)
  END IF
      OutData%NumTwrAlf = IntKiBuf( Int_Xferred ) 
      Int_Xferred   = Int_Xferred + 1
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! TwrAoA not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%TwrAoA)) DEALLOCATE(OutData%TwrAoA)
    ALLOCATE(OutData%TwrAoA(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%TwrAoA.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%TwrAoA)>0) OutData%TwrAoA = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%TwrAoA))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%TwrAoA)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! TwrCl not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%TwrCl)) DEALLOCATE(OutData%TwrCl)
    ALLOCATE(OutData%TwrCl(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%TwrCl.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%TwrCl)>0) OutData%TwrCl = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%TwrCl))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%TwrCl)
    DEALLOCATE(mask1)
  END IF
  ...
  ELSE
    ...
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! TwrCm not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%TwrCm)) DEALLOCATE(OutData%TwrCm)
    ALLOCATE(OutData%TwrCm(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%TwrCm.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%TwrCm)>0) OutData%TwrCm = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%TwrCm))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%TwrCm)
    DEALLOCATE(mask1)
  END IF
...
ENDIF
IF (ALLOCATED(SrcMiscData%X_TwrLift)) THEN
  i1_l = LBOUND(SrcMiscData%X_TwrLift,1)
  i1_u = UBOUND(SrcMiscData%X_TwrLift,1)
  IF (.NOT. ALLOCATED(DstMiscData%X_TwrLift)) THEN 
    ALLOCATE(DstMiscData%X_TwrLift(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%X_TwrLift.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstMiscData%X_TwrLift = SrcMiscData%X_TwrLift
ENDIF
IF (ALLOCATED(SrcMiscData%Y_TwrLift)) THEN
  i1_l = LBOUND(SrcMiscData%Y_TwrLift,1)
  i1_u = UBOUND(SrcMiscData%Y_TwrLift,1)
  IF (.NOT. ALLOCATED(DstMiscData%Y_TwrLift)) THEN 
    ALLOCATE(DstMiscData%Y_TwrLift(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%Y_TwrLift.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstMiscData%Y_TwrLift = SrcMiscData%Y_TwrLift
ENDIF
IF (ALLOCATED(SrcMiscData%M_Twr)) THEN
  i1_l = LBOUND(SrcMiscData%M_Twr,1)
  i1_u = UBOUND(SrcMiscData%M_Twr,1)
  IF (.NOT. ALLOCATED(DstMiscData%M_Twr)) THEN 
    ALLOCATE(DstMiscData%M_Twr(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%M_Twr.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstMiscData%M_Twr = SrcMiscData%M_Twr
ENDIF
...
ENDIF
IF (ALLOCATED(MiscData%X_TwrLift)) THEN
  DEALLOCATE(MiscData%X_TwrLift)
ENDIF
IF (ALLOCATED(MiscData%Y_TwrLift)) THEN
  DEALLOCATE(MiscData%Y_TwrLift)
ENDIF
IF (ALLOCATED(MiscData%M_Twr)) THEN
  DEALLOCATE(MiscData%M_Twr)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! X_TwrLift allocated yes/no
  IF ( ALLOCATED(InData%X_TwrLift) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! X_TwrLift upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%X_TwrLift)  ! X_TwrLift
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! Y_TwrLift allocated yes/no
  IF ( ALLOCATED(InData%Y_TwrLift) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! Y_TwrLift upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%Y_TwrLift)  ! Y_TwrLift
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! M_Twr allocated yes/no
  IF ( ALLOCATED(InData%M_Twr) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! M_Twr upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%M_Twr)  ! M_Twr
  END IF
  ...
  END IF
  IF ( .NOT. ALLOCATED(InData%X_TwrLift) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%X_TwrLift,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%X_TwrLift,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%X_TwrLift)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%X_TwrLift))-1 ) = PACK(InData%X_TwrLift,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%X_TwrLift)
  END IF
  IF ( .NOT. ALLOCATED(InData%Y_TwrLift) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%Y_TwrLift,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%Y_TwrLift,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%Y_TwrLift)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%Y_TwrLift))-1 ) = PACK(InData%Y_TwrLift,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%Y_TwrLift)
  END IF
  IF ( .NOT. ALLOCATED(InData%M_Twr) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%M_Twr,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%M_Twr,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%M_Twr)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%M_Twr))-1 ) = PACK(InData%M_Twr,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%M_Twr)
  END IF
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! X_TwrLift not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%X_TwrLift)) DEALLOCATE(OutData%X_TwrLift)
    ALLOCATE(OutData%X_TwrLift(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%X_TwrLift.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%X_TwrLift)>0) OutData%X_TwrLift = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%X_TwrLift))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%X_TwrLift)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! Y_TwrLift not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%Y_TwrLift)) DEALLOCATE(OutData%Y_TwrLift)
    ALLOCATE(OutData%Y_TwrLift(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%Y_TwrLift.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%Y_TwrLift)>0) OutData%Y_TwrLift = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%Y_TwrLift))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%Y_TwrLift)
    DEALLOCATE(mask1)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! M_Twr not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%M_Twr)) DEALLOCATE(OutData%M_Twr)
    ALLOCATE(OutData%M_Twr(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%M_Twr.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%M_Twr)>0) OutData%M_Twr = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%M_Twr))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%M_Twr)
    DEALLOCATE(mask1)
  END IF
...
IF (ALLOCATED(SrcParamData%TwrElev)) THEN
  i1_l = LBOUND(SrcParamData%TwrElev,1)
  i1_u = UBOUND(SrcParamData%TwrElev,1)
  IF (.NOT. ALLOCATED(DstParamData%TwrElev)) THEN 
    ALLOCATE(DstParamData%TwrElev(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%TwrElev.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%TwrElev = SrcParamData%TwrElev
ENDIF
...
ENDIF
    DstParamData%NumTwrAlf = SrcParamData%NumTwrAlf
IF (ALLOCATED(SrcParamData%TwrAoA)) THEN
  i1_l = LBOUND(SrcParamData%TwrAoA,1)
  i1_u = UBOUND(SrcParamData%TwrAoA,1)
  IF (.NOT. ALLOCATED(DstParamData%TwrAoA)) THEN 
    ALLOCATE(DstParamData%TwrAoA(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%TwrAoA.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%TwrAoA = SrcParamData%TwrAoA
ENDIF
IF (ALLOCATED(SrcParamData%TwrCl)) THEN
  i1_l = LBOUND(SrcParamData%TwrCl,1)
  i1_u = UBOUND(SrcParamData%TwrCl,1)
  IF (.NOT. ALLOCATED(DstParamData%TwrCl)) THEN 
    ALLOCATE(DstParamData%TwrCl(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%TwrCl.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%TwrCl = SrcParamData%TwrCl
ENDIF
...
ENDIF
IF (ALLOCATED(SrcParamData%TwrCm)) THEN
  i1_l = LBOUND(SrcParamData%TwrCm,1)
  i1_u = UBOUND(SrcParamData%TwrCm,1)
  IF (.NOT. ALLOCATED(DstParamData%TwrCm)) THEN 
    ALLOCATE(DstParamData%TwrCm(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%TwrCm.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%TwrCm = SrcParamData%TwrCm
ENDIF
...
! 
  ErrStat = ErrID_None
  ErrMsg  = ""
IF (ALLOCATED(ParamData%TwrElev)) THEN
  DEALLOCATE(ParamData%TwrElev)
ENDIF
IF (ALLOCATED(ParamData%TwrDiam)) THEN
  DEALLOCATE(ParamData%TwrDiam)
ENDIF
IF (ALLOCATED(ParamData%TwrAoA)) THEN
  DEALLOCATE(ParamData%TwrAoA)
ENDIF
IF (ALLOCATED(ParamData%TwrCl)) THEN
  DEALLOCATE(ParamData%TwrCl)
ENDIF
IF (ALLOCATED(ParamData%TwrCd)) THEN
  DEALLOCATE(ParamData%TwrCd)
ENDIF
IF (ALLOCATED(ParamData%TwrCm)) THEN
  DEALLOCATE(ParamData%TwrCm)
ENDIF
...
! 
   ...
   ErrMsg  = ""
      CALL MeshCopy( SrcOutputData%TowerLoad, DstOutputData%TowerLoad, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
      CALL MeshCopy( SrcOutputData%TowerLiftLoad, DstOutputData%TowerLiftLoad, CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
...
! 
  ...
  CALL MeshDestroy( OutputData%TowerLiftLoad, ErrStat, ErrMsg )
 ...
 ! buffers to store subtypes, if any
  ...
  Int_BufSz  = 0
   ! Allocate buffers for subtypes, if any (we'll get sizes from these) 
      ...
      IF(ALLOCATED(Int_Buf)) THEN ! TowerLoad
         ...
         DEALLOCATE(Int_Buf)
      END IF
      Int_BufSz   = Int_BufSz + 3  ! TowerLiftLoad: size of buffers for each call to pack subtype
      CALL MeshPack( InData%TowerLiftLoad, Re_Buf, Db_Buf, Int_Buf, ErrStat2, ErrMsg2, .TRUE. ) ! TowerLiftLoad 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN

      IF(ALLOCATED(Re_Buf)) THEN ! TowerLiftLoad
         Re_BufSz  = Re_BufSz  + SIZE( Re_Buf  )
         DEALLOCATE(Re_Buf)
      END IF
      IF(ALLOCATED(Db_Buf)) THEN ! TowerLiftLoad
         Db_BufSz  = Db_BufSz  + SIZE( Db_Buf  )
         DEALLOCATE(Db_Buf)
      END IF
      IF(ALLOCATED(Int_Buf)) THEN ! TowerLiftLoad
         Int_BufSz = Int_BufSz + SIZE( Int_Buf )
         DEALLOCATE(Int_Buf)
      END IF
  ...
  Int_Xferred = 1
      ...
      ELSE
        IntKiBuf( Int_Xferred ) = 0; Int_Xferred = Int_Xferred + 1
      ENDIF
      CALL MeshPack( InData%TowerLiftLoad, Re_Buf, Db_Buf, Int_Buf, ErrStat2, ErrMsg2, OnlySize ) ! TowerLiftLoad 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
 ...
 ! buffers to store meshes, if any
  ...
  Int_Xferred  = 1
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL MeshUnpack( OutData%TowerLiftLoad, Re_Buf, Db_Buf, Int_Buf, ErrStat2, ErrMsg2 ) ! TowerLiftLoad 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
 ...
 t_out = tin_out - tin(1)
   ...
   END IF
      CALL MeshExtrapInterp1(y1%TowerLoad, y2%TowerLoad, tin, y_out%TowerLoad, tin_out, ErrStat2, ErrMsg2 )
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
      CALL MeshExtrapInterp1(y1%TowerLiftLoad, y2%TowerLiftLoad, tin, y_out%TowerLiftLoad, tin_out, ErrStat2, ErrMsg2 )
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
 ...
 t_out = tin_out - tin(1)
   ...
   END IF
      CALL MeshExtrapInterp2(y1%TowerLoad, y2%TowerLoad, y3%TowerLoad, tin, y_out%TowerLoad, tin_out, ErrStat2, ErrMsg2 )
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
      CALL MeshExtrapInterp2(y1%TowerLiftLoad, y2%TowerLiftLoad, y3%TowerLiftLoad, tin, y_out%TowerLiftLoad, tin_out, ErrStat2, ErrMsg2 )
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
IF (ALLOCATED(y_out%BladeLoad) .AND. ALLOCATED(y1%BladeLoad)) THEN
  DO i01 = LBOUND(y_out%BladeLoad,1),UBOUND(y_out%BladeLoad,1)
