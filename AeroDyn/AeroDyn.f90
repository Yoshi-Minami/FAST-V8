...
SUBROUTINE ADTwr_CalcOutput(p, u, m, y, ErrStat, ErrMsg )
   ...
   real(ReKi)                                   :: tmp(3)
   real(ReKi)                                   :: AoA,TwrCl,TwrCd,TwrCm,LenSec
   ...
   do j=1,p%NumTwrNds
      ...
      AoA = atan2(VL(2),VL(1))*R2D
      
      if (j.eq.9) write(10000,*) j, AoA
      
      CALL TwrAirFoilCoef_Calc(AoA,p,TwrCl,TwrCd,TwrCm)
      m%W_Twr(j)  =  TwoNorm( VL )            ! relative wind speed normal to the tower at node j
      
      ! calculate aerodynamic forces and moment
      q1     = 0.5 * p%AirDens * m%W_Twr(j) * p%TwrDiam(j) * TwrCd   ! drag force
      q2     = 0.5 * p%AirDens * m%W_Twr(j) * p%TwrDiam(j) * TwrCl   ! lift force, q=1/2*rho*v**2*c*Cl
            
     ! force per unit length of the jth node in the tower, drag force
      tmp(1) = q2 * VL(2) - q1 * VL(1)
      tmp(2) = q2 * VL(1) + q1 * VL(2)
      tmp(3) = 0.0_ReKi
      
      m%X_Twr(j) =-q1 * VL(1)
      m%Y_Twr(j) = q1 * VL(2)
      m%X_TwrLift(j) = q2 * VL(2)
      m%Y_TwrLift(j) = q2 * VL(1)
...
END SUBROUTINE ADTwr_CalcOutput
   ...
   !-------------------------------------------------------------------------------------------------
   ! Find the lift and drag coefficients, written by Yingyi Liu, Kyushu University, Jan. 30, 2017
   !-------------------------------------------------------------------------------------------------    
!----------------------------------------------------------------------------------------------------------------------------------
